...
OPMM2GUI.Name = "OP MM2 GUI"
OPMM2GUI.Parent = Main
OPMM2GUI.BackgroundColor3 = Color3.new(0.788235, 0.14902, 1)
OPMM2GUI.Position = UDim2.new(0, 0, 0.501687825, 0)
OPMM2GUI.Size = UDim2.new(0, 200, 0, 50)
OPMM2GUI.Font = Enum.Font.Cartoon
OPMM2GUI.Text = "MM2 GUI OP"
OPMM2GUI.TextColor3 = Color3.new(0, 0, 0)
OPMM2GUI.TextScaled = true
OPMM2GUI.TextSize = 14
OPMM2GUI.TextWrapped = true
OPMM2GUI.MouseButton1Down:connect(function()
...
RED.Name = "RED"
RED.Parent = Main
RED.BackgroundColor3 = Color3.new(0.788235, 0.14902, 1)
RED.Position = UDim2.new(0.341545671, 0, 0.503687799, 0)
RED.Size = UDim2.new(0, 200, 0, 50)
RED.Font = Enum.Font.Cartoon
RED.Text = "RED MM2 GUI"
...
RACIST.Name = "RACIST"
RACIST.Parent = Main
RACIST.BackgroundColor3 = Color3.new(0.788235, 0.14902, 1)
RACIST.Position = UDim2.new(0.68331778, 0, 0.503687799, 0)
RACIST.Size = UDim2.new(0, 200, 0, 50)
RACIST.Font = Enum.Font.Cartoon
RACIST.Text = "RACIST DOLPHIN GUI"
RACIST.TextColor3 = Color3.new(0, 0, 0)
RACIST.TextScaled = true
RACIST.TextSize = 14
RACIST.TextWrapped = true
RACIST.MouseButton1Down:connect(function()
...
if Bullshit.ESPLength > 2048 then
	Bullshit.ESPLength = 2048
end
...
function GetTeamColor(Plr)
	local PickedColor = Color3.new(1, 1, 1)
...
function CreateESP(Plr)
	if Plr ~= nil then
		...
		TxtPos.Position = UDim2.new(0, 0, 0, -15)
...
-- Old update tracers code
--[[function UpdateTracers()
	if Bullshit.TracersEnabled then
		if MyChar then
			for _, v in next, Plrs:GetPlayers() do -- lag
				local MyTor = MyChar:FindFirstChild("HumanoidRootPart")
				local GetTor = TracerData[v.Name]
				if MyTor and GetTor ~= nil then
					local Dist = (MyTor.Position - GetTor.Position).magnitude
					if Dist < Bullshit.TracersLength then
						if not Bullshit.PlaceTracersUnderCharacter then
							local R = MyCam:ScreenPointToRay(MyCam.ViewportSize.X / 2, MyCam.ViewportSize.Y, 0)
							Dist = (R.Origin - (GetTor.Position - Vector3.new(0, 3, 0))).magnitude
							Tracers[v.Name].Transparency = 0
							Tracers[v.Name].Size = Vector3.new(0.05, 0.05, Dist)
							Tracers[v.Name].CFrame = CFrame.new(R.Origin, (GetTor.Position - Vector3.new(0, 4.5, 0))) * CFrame.new(0, 0, -Dist / 2)
							Tracers[v.Name].BrickColor = BrickColor.new(GetTeamColor(v))
							Tracers[v.Name].Mesh.Scale = Vector3.new(0.03, 0.03, Tracers[v.Name].Size.Z / Dist)
						else
							Dist = (MyTor.Position - (GetTor.Position - Vector3.new(0, 3, 0))).magnitude
							Tracers[v.Name].Transparency = 0
							Tracers[v.Name].Size = Vector3.new(0.3, 0.3, Dist)
							Tracers[v.Name].CFrame = CFrame.new(MyTor.Position - Vector3.new(0, 3, 0), (GetTor.Position - Vector3.new(0, 4.5, 0))) * CFrame.new(0, 0, -Dist / 2)
							Tracers[v.Name].BrickColor = BrickColor.new(GetTeamColor(v))
							Tracers[v.Name].Mesh.Scale = Vector3.new(0.3, 0.3, Tracers[v.Name].Size.Z / Dist)
						end
					else
...
function UpdateChams(Plr)
	...
	if Find then
		for _, v in next, Find:GetChildren() do
			local GetChar = Plr.Character
			if GetChar then
				local GetTor = GetChar:FindFirstChild("HumanoidRootPart")
				local MyTor = MyChar:FindFirstChild("HumanoidRootPart")
				if GetTor and MyTor then
...
Run:BindToRenderStep("UpdateInfo", Enum.RenderPriority.First.Value, function()
	MyChar = MyPlr.Character
	if Bullshit.DebugInfo then
		DebugMenu["Main"].Visible = true
		...
		for i = 1, #Bullshit.FPSAverage do -- Thought for sure this would slow down the update, but nope. Everything seems to running smooth still.
			sum = sum + Bullshit.FPSAverage[i]
		end
		DebugMenu["FPS"].Text = "FPS: " .. tostring(fps) .. " Average: " .. string.format("%.0f", (sum / #Bullshit.FPSAverage))
		if (tick() - FPSTick) >= 10 then
			FPSTick = tick()
			Bullshit.FPSAverage = { } -- We don't need the for loop going through 1 million numbers.
		end
		LastTick = tick()
		...
		if #Teams:GetChildren() > 0 then
			...
			DebugMenu["PlayerSelected"].Text = "Closest Player: " .. tostring(Bullshit.ClosestEnemy)
		end


		if Bullshit.ClosestEnemy ~= nil then
			if Bullshit.ClosestEnemy.Team ~= nil then
				DebugMenu["PlayerTeam"].Text = "Team: " .. tostring(Bullshit.ClosestEnemy.Team.Name)
			else
				DebugMenu["PlayerTeam"].Text = "Team: nil (Free for All)"
			end
		end
		if GetChar then
			local Hum = GetChar:FindFirstChild("Humanoid")
			local Tor = GetChar:FindFirstChild("HumanoidRootPart")
			if Hum then
				DebugMenu["PlayerHealth"].Text = "Health: " .. string.format("%.0f", Hum.Health)
			end
			if MyTor and Tor then
...
Run:BindToRenderStep("Aimbot", Enum.RenderPriority.First.Value, function()
	if Bullshit.AimbotEnabled and Bullshit.Aimbot then
		if Bullshit.ClosestEnemy ~= nil then
			local GetChar = Bullshit.ClosestEnemy.Character
			if MyChar and GetChar then
				...
				if MyHead and MyTor and MyHum and GetHead and GetTor and GetHum then
					if MyHum.Health > 1 and GetHum.Health > 1 then
						MyPlr.CameraMode = Enum.CameraMode.LockFirstPerson
						MyCam.CFrame = CFrame.new(MyHead.CFrame.p, GetHead.CFrame.p - Vector3.new(0, 0.05, 0))
					end
				end
			end
		end
...
Close.Position = UDim2.new(0.818037987, 0, 0, 0)
Close.Size = UDim2.new(0, 115, 0, 65)
...
TextLabel_2.BackgroundColor3 = Color3.new(0.129412, 0.129412, 0.129412)
TextLabel_2.BorderSizePixel = 0
TextLabel_2.Position = UDim2.new(0, 0, 0.744680822, 0)
TextLabel_2.Size = UDim2.new(0, 632, 0, 84)
TextLabel_2.Font = Enum.Font.Cartoon
TextLabel_2.Text = "GUI BY LUDDE2K ON V3RM!"
TextLabel_2.TextColor3 = Color3.new(0.972549, 0.0745098, 1)
TextLabel_2.TextScaled = true
...
OpenFrame.Position = UDim2.new(0.936668754, 0, 0.579852581, 0)
OpenFrame.Size = UDim2.new(0, 100, 0, 50)

Open.Name = "Open"
Open.Parent = OpenFrame
Open.BackgroundColor3 = Color3.new(0.737255, 0.921569, 1)
Open.Position = UDim2.new(-0.00999999978, 0, -0.140000001, 0)
Open.Size = UDim2.new(0, 126, 0, 57)
